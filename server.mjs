...
async function deliverSms(to, message) {
  ...
  if (!accountSid || !authTokenValue || !from) {
    return {
      status: "queued",
      label: "SMS servisi bağlı değil",
      detail: "TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN ve TWILIO_FROM_NUMBER tanımlanınca gerçek SMS gönderilir.",
    };
  }
  ...
  if (!smsResponse.ok) {
    return {
      status: "failed",
      label: "SMS gönderilemedi",
      detail: result.message || "SMS sağlayıcı gönderimi reddetti.",
    };
  }
  ...
  db.notifications = [
    {
      ...
      message,
      status: ["sent", "failed"].includes(smsResult.status) ? smsResult.status : "queued",
...
async function handleCreateAppointment(request, response) {
  ...
  sendJson(response, 201, {
    message: smsResult.status === "sent"
      ? "Randevu oluşturuldu ve SMS gönderildi."
      : "Randevu oluşturuldu. SMS sağlayıcı ayarı bekleniyor.",
    appointmentId: appointment.id,
...
async function handleUpdateSettings(request, response) {
  ...
  const removedBarberIds = db.settings.barbers
    .map((barber) => barber.id)
    .filter((id) => !nextBarberIds.has(id));
  const hasAppointmentsForRemovedBarber = db.appointments.some((appointment) =>
    removedBarberIds.includes(appointment.barberId),
  );

  if (hasAppointmentsForRemovedBarber) {
    sendJson(response, 409, {
      message: "Randevusu bulunan berber silinemez. Önce o berberin randevularını iptal et.",
    });
    return;
  }
...
server.listen(port, host, () => {
  ...
  console.log(`Phone/LAN access: http://YOUR_COMPUTER_IP:${port}`);
});
